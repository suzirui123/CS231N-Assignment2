...
class FullyConnectedNet(object):
  ...
  def loss(self, X, y=None):
    ...
    # layer, etc.                                                              #
    ############################################################################
    h, cache1, cache2, cache3, cache4, out, bn = { }, { }, { }, { }, { }, { }, { }
    out[0] = X
    for i in xrange(self.num_layers - 1):
        ...
        if self.use_batchnorm:
            ...
            out[i + 1], cache3[i + 1] = affine_relu_forward(out[i], W, b)
            if self.use_dropout:
                out[i + 1], cache4[i] = dropout_forward(out[i + 1], self.dropout_param)
    ...
    for i in xrange(t):    
        if self.use_batchnorm:
            ...
            if self.use_dropout:    
                    ddrop[t-1-i] = dropout_backward(dout[t-i], cache4[t-1-i])    
                    dout[t-i] = ddrop[t-1-i]     
            dout[t-i-1], grads['W'+str(t-i)], grads['b'+str(t-i)] = affine_relu_backward(dout[t-i], cache3[t-i])
    for i in xrange(self.num_layers):
